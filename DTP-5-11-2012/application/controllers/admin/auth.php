...
class Auth extends CI_Controller
{
	...
	function login()
	{
		if ($this->tank_auth->is_admin_login()) {									// logged in
			...
			$this->load->view('admin/auth/auth_header');
	...
	/**
	 * Register user on behalf of admin
	 *
	 * @return void
	 */
	function registerbyadmin()
	{
		if (!$this->tank_auth->is_admin_login()) {									// logged in
			...
			else
			{
				...
				$data['mb_data'] = $this->load->view('admin/addusers', null, true);
				$this->load->view('admin/mainbody', $data);
	 ...
	 * @return void
	 */
	function edituserbyadmin($userid = 1)
	{
		
		$this->load->Model('admin/UserManager');
		
		$data['userData'] = $this->UserManager->getuserdata($userid);
		...
		else 
		{
			if($this->input->post('post'))
			{
				...
				if ($this->form_validation->run()) {								// validation ok
					...
					if (!is_null($data = $this->tank_auth->update_userbyadmin(
							$userid,
							$this->form_validation->set_value('firstname'),
							$this->form_validation->set_value('lastname'),
							$this->form_validation->set_value('email'),
						...
						redirect(site_url('admin/usermanagement/showusers'));
				...
				{
					$this->load->view('admin/header');
					$data['mb_data'] = $this->load->view('admin/editusers', $null, true);
					$this->load->view('admin/mainbody', $data);
					$this->load->view('admin/footer');
				}
			}
			...
			{
				$this->load->view('admin/header');
				$data['mb_data'] = $this->load->view('admin/editusers', $data, true);
				$this->load->view('admin/mainbody', $data);
				$this->load->view('admin/footer');
			}
		}
	...
	function register()
	{
		if ($this->tank_auth->is_admin_login()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_admin(
	...
	{
		if (!$this->tank_auth->is_admin_login()) {								// not logged in or not activated
			redirect('/admin/auth/login/');
