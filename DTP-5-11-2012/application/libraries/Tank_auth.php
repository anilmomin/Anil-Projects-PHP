...
class Tank_auth
{
    ...
    /**
    	 ...
    	 * (user exists and activated, password is correct), otherwise FALSE.
    	 *
    	 * @param	string	(Hash code from the activation mail)
    	 */
    function loginByHash($hash)
    {
        if (strlen($hash) > 0 ) {

            if (!is_null($user = $this->ci->users->get_user_by_hash($hash))) {	// login ok
    ...
    /**
     * Updates the user details from admin
     ...
     * @return	array
     */
    function update_userbyadmin($user_id, $first_name, $last_name, $email = null, $password = null, $dob = null, $is_active, $address = null)
    {
    	$banuser = 0;
    	$activated = 1;
    	...
    	if (!is_null($res = $this->ci->users->update_user($data, $user_id))) {
    		$this->ci->users->update_address($address, $user_id);
    		return $res;
    	}
     ...
     * @return	array
     */
    function update_user($user_id, $first_name, $last_name, $password, $dob = null)
    {
	...
	function is_logged_in($activated = TRUE)
	{
        ...
        $active = ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);

        $admin  = $this->ci->session->userdata('is_admin');
		
        if( ($this->ci->session->userdata('status') === $active) && ($admin == 0))
        {
            return $active;
        }
        else
	 ...
	 * @return	bool
	 */
    function is_admin_login($admin = TRUE)
	{
        return $this->ci->session->userdata('is_admin') === ($admin ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);;
	}
	 ...
	 * Subscription
	 */
	function create_subsciption($first_name, $last_name, $email)
	{
		if (!$this->ci->users->is_email_available($email)) {
			...
			$res = $this->ci->users->create_subscriber($data, TRUE);
			return $res;
		}
	 ...
	 * @return	array
	 */
    function create_user($username, $first_name, $last_name, $email, $password, $email_activation, $is_sampleuser = 0, $dob = null)
	{
		if (!$this->ci->users->is_email_available($email)) {
			$this->error = array('email' => 'auth_email_in_use');
	...
	/**
		 ...
		 * @return	array
		 */
	function create_admin($username, $email, $password, $email_activation)
	{
		if (!$this->ci->users->is_email_available($email)) {
			$this->error = array('email' => 'auth_email_in_use');
	...
	/**
	 * Unsubscribe user from the site (when user is not logged in)
	 */
