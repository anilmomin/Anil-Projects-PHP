...
class PayPal_IPN
{
	...
	function paypal_form($form_name='paypal_form') 
	{
		...
		$str = '';
		$str .= '<form style="display: none;" id="'.$form_name.'" method="post" action="'.$this->ipnURL.'" name="'.$form_name.'">' . "\n";
	...
	// Image button for the paypal
	function image()
	{
		
		$this->submit_btn = '<input type="image" name="submit" src="'.base_url().'/assets/images/btncheckoutnow.png" alt="Checkout with Paypal" border="0" />';
	}
    ...
    // Updates the quantity of wine present in the database
    
 	public function updateQuantity()
    {
    	$this->_ci->load->Model('admin/WineManager');
    	$this->_ci->load->Model('Ipn_order_model');
		$updateQuantity = array();
		$wineIds = array();
		
		$cartcontents = $this->orderItems;
		
		// get all the wine ids from the ipn response
		foreach ($cartcontents as $cartitem)
			$wineIds[] = $cartitem['item_number'];
		
		$wineQuantity = $this->_ci->WineManager->getwineByIds($wineIds); // get all the wines of the cart
		array_multisort($wineIds, $cartcontents);
		$cartcontents = array_values($cartcontents);
		
		for ($i = 0; $i < count($wineQuantity); $i++)
		{
			$newQuantity = $wineQuantity[$i]->wineQuantity - $cartcontents[$i]['quantity'];
			if($newQuantity >= 0)
				$updateQuantity = array('wineId' => $wineQuantity[$i]->wineId, 'wineQuantity' => $newQuantity);
		
			$this->_ci->WineManager->update($updateQuantity);
		}
